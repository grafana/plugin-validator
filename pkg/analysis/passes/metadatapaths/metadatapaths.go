...
func checkMetadataPaths(pass *analysis.Pass) (interface{}, error) {
	...
	for _, path := range paths {
		...
		if err != nil {
			pass.Reportf(pass.AnalyzerName, invalidPath, fmt.Sprintf("plugin.json: invalid %s path: %s", path.kind, path.path))
			continue
		} else {
			if invalidPath.ReportAll {
				invalidPath.Severity = analysis.OK
				pass.Reportf(pass.AnalyzerName, invalidPath, fmt.Sprintf("plugin.json: valid %s path: %s", path.kind, path.path))
			}
		}

		if u.IsAbs() {
			pass.Reportf(pass.AnalyzerName, pathRelativeToMetadata, fmt.Sprintf("plugin.json: %s path should be relative to plugin.json: %s", path.kind, path.path))
			continue
		} else {
			if pathRelativeToMetadata.ReportAll {
				pathRelativeToMetadata.Severity = analysis.OK
				pass.Reportf(pass.AnalyzerName, pathRelativeToMetadata, fmt.Sprintf("plugin.json: %s path is relative to plugin.json: %s", path.kind, path.path))
			}
		}

		if strings.HasPrefix(path.path, ".") || strings.HasPrefix(path.path, "/") {
			pass.Reportf(pass.AnalyzerName, invalidRelativePath, fmt.Sprintf("plugin.json: relative %s path should not start with '.' or '/': %s", path.kind, path.path))
			continue
		} else {
			if invalidRelativePath.ReportAll {
				invalidRelativePath.Severity = analysis.OK
				pass.Reportf(pass.AnalyzerName, invalidRelativePath, fmt.Sprintf("plugin.json: relative %s path does not start with '.' or '/': %s", path.kind, path.path))
			}
		}
		...
		if archiveDir != "" {
			...
			if err != nil {
				if os.IsNotExist(err) {
					pass.Reportf(pass.AnalyzerName, pathNotExists, fmt.Sprintf("plugin.json: %s path doesn't exists: %s", path.kind, path.path))
				}
			}
		}
	}
